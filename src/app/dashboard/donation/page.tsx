...
export default function DonationPage() {
  ...
  const { data, isLoading } = useGetMyDonationsQuery({ page, limit });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed border-border py-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            When you give, your receipts show up here.
          </p>
          <Link
            href="/donate"
            className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-emerald-600 px-5 py-2 text-sm font-semibold text-white hover:bg-emerald-700 transition-colors"
          >
                  ...
                  {rows.map((d: ApiDonation) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-block rounded-full px-2 py-0.5 text-[11px] font-semibold ${statusStyle[d.status]}`}
                        >
                          {d.status}
                        </span>
                      </td>
