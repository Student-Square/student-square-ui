...
// ─── Category selector ────────────────────────────────────────────────────────
function CategorySelector({ value, onChange }: { value: string; onChange: (slug: string, name: string) => void }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      refetch(); onChange(cat.slug, cat.name);
            ...
            <div className="max-h-48 overflow-y-auto p-1">
              {categories.map((cat) => (
                <button key={cat.id} type="button" onClick={() => { onChange(cat.slug, cat.name); setOpen(false); }}
                  ...
                  {cat.name} {cat.slug === value && <Check className="h-3.5 w-3.5" />}
                </button>
              ))}
            </div>
            <div className="border-t border-border p-2">
              {!showCreate ? (
                <button type="button" onClick={() => setShowCreate(true)} className="w-full flex items-center gap-2 px-3 py-2 text-sm rounded-lg text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 font-semibold transition-colors">
                ...
                <div className="space-y-2 p-1">
                  <input autoFocus type="text" value={newCatName}
                    ...
                    placeholder="Category name…" className="w-full px-3 py-2 text-sm rounded-lg bg-background border border-border focus:border-emerald-500 focus:outline-none" />
                  {dupError && <p className="text-xs text-red-500 px-1">{dupError}</p>}
                  {newCatName && !dupError && <p className="text-[10px] text-muted-foreground px-1">Slug: <span className="font-mono text-emerald-600">{slugify(newCatName)}</span></p>}
                  <div className="flex gap-2">
...
export default function EditBlogPage() {
  ...
  const { data: post, isLoading, isError } = useAdminGetBlogQuery(id);
  const [updateBlog, { isLoading: saving }] = useAdminUpdateBlogMutation();
  ...
  useEffect(() => {
    if (post && !initialized) {
      ...
      setTags(post.tags.map((t) => t.tag.slug));
  ...
  const buildPayload = (): UpdateBlogPostInput => {
    const payload: UpdateBlogPostInput = { title, excerpt, body, categorySlug, tagSlugs: tags, isFeatured };
    payload.coverImageId = coverImageId || null;
    return payload;
  };
  ...
  const handleSave = async () => {
    if (!isValid) return;
    try {
      await updateBlog({ id, data: buildPayload() }).unwrap();
  ...
  if (isLoading) {
    return <div className="flex items-center gap-2 text-sm text-muted-foreground py-20"><Loader2 className="h-4 w-4 animate-spin" /> Loading post…</div>;
  }
  if (isError || !post) {
    return <p className="text-sm text-red-600 py-10">Post not found.</p>;
  }

  const displayAuthor = post.displayAuthorName ?? post.author?.fullName ?? user?.fullName ?? "Student Square";
      ...
      <div className="sticky top-0 z-40 bg-background/95 backdrop-blur border-b border-border px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-sm font-bold text-foreground">Edit Post</h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="hidden sm:flex items-center gap-1.5 text-[11px] text-muted-foreground bg-muted px-2.5 py-1 rounded-full">
            By <span className="font-semibold text-foreground">{displayAuthor}</span>
          </span>
          ...
          <div className={`hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold ${
            ...
            <span className={`w-1.5 h-1.5 rounded-full ${post.status === "PUBLISHED" ? "bg-emerald-500" : post.status === "ARCHIVED" ? "bg-muted-foreground" : "bg-amber-400"}`} />
            ...
            <motion.div key="write" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  {initialized && (
                    <RichTextEditor content={body} onChange={setBody} placeholder="Write your post content here…" />
                  )}
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="rounded-xl border border-dashed border-border bg-card/40 px-4 py-3 flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full shrink-0 ${post.status === "PUBLISHED" ? "bg-emerald-400" : post.status === "ARCHIVED" ? "bg-muted-foreground" : "bg-amber-400"}`} />
                  <div>
                    <p className="text-xs font-semibold text-foreground">{post.status.charAt(0) + post.status.slice(1).toLowerCase()}</p>
                    <p className="text-[11px] text-muted-foreground">
                      {post.status === "PUBLISHED" ? `Published ${formatDate(post.publishedAt)}` : post.status === "ARCHIVED" ? "Archived" : "Not published yet"}
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-border bg-card/40 px-4 py-3">
                  ...
                  <p className="text-sm font-semibold text-foreground">{displayAuthor}</p>
                </div>
